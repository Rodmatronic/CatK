...
static void liballoc_dump()
{
	...
	debug( "liballoc: System memory allocated: %d bytes\n", l_allocated );
	debug( "liballoc: Memory in used (malloc'ed): %d bytes\n", l_inuse );
	debug( "liballoc: Warning count: %d\n", l_warningCount );
	debug( "liballoc: Error count: %d\n", l_errorCount );
	debug( "liballoc: Possible overruns: %d\n", l_possibleOverruns );

#ifdef DEBUG
		while ( maj != NULL )
		{
			debug( "liballoc: %x: total = %d, used = %d\n",
						maj, 
						maj->size,
						maj->usage );
...
static struct liballoc_major *allocate_new_page( unsigned int size )
{
	...
	struct liballoc_major *maj;
		...
		if ( maj == NULL ) 
		{
			l_warningCount += 1;
			#if defined DEBUG || defined INFO
			debug( "liballoc: WARNING: liballoc_alloc( %d ) return NULL\n", st );
			#endif
		...
		#ifdef DEBUG
		debug( "liballoc: Resource allocated %x of %d pages (%d bytes) for %d size.\n", maj, st, maj->size, size );

		debug( "liballoc: Total memory usage = %d KB\n",  (int)((l_allocated / (1024))) );
		#endif
...
void * malloc(size_t req_size)
{
	...
	#endif
	#if defined DEBUG || defined INFO
	debug( "liballoc: WARNING: malloc( %d ) returning NULL.\n", size);
	liballoc_dump();
	#endif
	return NULL;
}
...
void free(void *ptr)
{
	...
	if ( ptr == NULL ) 
	{
		l_warningCount += 1;
		#if defined DEBUG || defined INFO
		debug( "liballoc: WARNING: free( NULL ) called from %x\n",
							__builtin_return_address(0) );
		#endif
		return;
	}
	...
	if ( min->magic != LIBALLOC_MAGIC ) 
	{
		...
		{
			#if defined DEBUG || defined INFO
			debug( "liballoc: ERROR: multiple free() attempt on %x from %x.\n", 
									ptr,
									__builtin_return_address(0) );
			#endif
		}
		...
		{
			#if defined DEBUG || defined INFO
			debug( "liballoc: ERROR: Bad free( %x ) called from %x\n",
								ptr,
								__builtin_return_address(0) );
			#endif
		}
